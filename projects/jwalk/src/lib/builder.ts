...
export type Validation = "none" | "step" | "full";
...
export class Builder<Context, Primes extends string, Types> {
    ...
    type<Type extends string, const Detail>(
        type: ValidateAlias<Type, "ROOT" | GroupType, Primes | keyof Types>,
        detail: ValidateType<Detail, Primes> extends readonly []
            ? { __error: `${Type} requires at least one alias type` }
            : ValidateType<Detail, Primes>
    ): (Buildify & {
        context: Context;
        primes: Primes | Type;
        ...
        if (null != type && Array.isArray(detail)) {
            check.type(
                type as string,
    ...
    node<Key extends string, const Type, const Content, const Refs>(
        ...
        if (null != key && null != type && null != detail) {
            ...
            return new Builder(this.#memoizer, this.#validation, this.#primes, {
                ...this.#nodes,
                [key as string]: meta
            }) as (Buildify & {
                context: Context;
